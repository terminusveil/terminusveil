...
describe("emptyCopy", () => {
  ...
  it("due with a move read after the process date says when the multiplier moved", () => {
    ...
    assert.equal(copy, "Multiplier moved Fri 04 Sep, 11:10 ET, 1× → 1.002208×. Issuer still lists the action.");
  });
    ...
    const copy = emptyCopy(
      {
        ...
        action: CASH,
        effectiveAtIso: CASH.terminusIso,
        lastMove: { ...MOVED, effectiveAtIso: "2025-09-04T15:33:20.000Z", atIso: "2025-09-04T15:23:20.000Z" },
      },
      Date.parse("2026-09-04T22:00:00Z"),
    );
    assert.equal(copy, "Process date passed. No multiplier move read yet.");
  ...
  it("does not call a move that takes effect after the clock, and uses the block time when no effective time was read", () => {
    ...
    assert.equal(emptyCopy({ ...row, lastMove: MOVED }, NOW), "Process date passed. No multiplier move read yet.");
    assert.equal(
      emptyCopy({ ...row, lastMove: { ...MOVED, effectiveAtIso: null } }, Date.parse("2026-09-04T22:00:00Z")),
      "Multiplier moved Fri 04 Sep, 11:00 ET, 1× → 1.002208×. Issuer still lists the action.",
    );
    assert.equal(
      emptyCopy({ ...row, lastMove: { ...MOVED, effectiveAtIso: null, atIso: null } }, Date.parse("2026-09-04T22:00:00Z")),
  ...
  it("a veiled cash dividend is pending; the desk has no evidence of payment", () => {
    const copy = emptyCopy({
      ...
      staged: null,
      action: { ...CASH, processDate: { year: 2026, month: 9, day: 10 }, terminusIso: "2026-09-10T13:30:00.000Z" },
      effectiveAtIso: "2026-09-10T13:30:00.000Z",
      lastMove: null,
    });
    assert.equal(copy, "UPS veiled until Thu 10 Sep, 09:30 ET. Cash dividend $1.64 pending. Live 1×.");
  });
  ...
  it("prints a sub-1× staged figure from wei", () => {
    ...
    assert.equal(copy, "UPS veiled until Thu 10 Sep, 09:30 ET. Live 1×. Staged 0.05×. Staged is not live.");
  });
...
describe("resolveState", () => {
  ...
  it("is veiled when a different multiplier is staged for the future", () => {
    assert.equal(resolveState({ ...base, staged: "2000000000000000000", effectiveAtMs: NOW + 3_600_000 }), "veiled");
  });
  it("is due when the staged multiplier's effective time has passed", () => {
    assert.equal(resolveState({ ...base, staged: "2000000000000000000", effectiveAtMs: NOW - 60_000 }), "due");
  });
  ...
  it("is due when a figure is staged with no chain time and the issuer's process date passed", () => {
    assert.equal(
      resolveState({ ...base, staged: "2000000000000000000", actionInProgress: true, actionTerminusMs: NOW - 60_000 }),
      "due",
    );
  });
  it("treats a staged value equal to live as not staged", () => {
    assert.equal(resolveState({ ...base, staged: "1000000000000000000", effectiveAtMs: NOW + 3_600_000 }), "open");
  });
  it("treats a sub-1× wei staged value as staged", () => {
    assert.equal(resolveState({ ...base, staged: "50000000000000000", effectiveAtMs: NOW + 3_600_000 }), "veiled");
  });
  it("is veiled on an in-progress issuer action with terminus ahead", () => {
    assert.equal(resolveState({ ...base, actionInProgress: true, actionTerminusMs: NOW + 86_400_000 }), "veiled");
  });
  it("is due on an in-progress issuer action whose terminus passed", () => {
    assert.equal(resolveState({ ...base, actionInProgress: true, actionTerminusMs: NOW - 86_400_000 }), "due");
  });
  ...
  it("is paused when the oracle is paused, even with a staged figure", () => {
    assert.equal(resolveState({ ...base, oraclePaused: true, staged: "2000000000000000000", effectiveAtMs: NOW + 1 }), "paused");
  });
  it("is not paused when the pause flag was not read", () => {
    assert.equal(resolveState({ ...base, oraclePaused: null }), "open");
    assert.equal(resolveState({ ...base, oraclePaused: null, staged: "2000000000000000000", effectiveAtMs: NOW + 1 }), "veiled");
  });
